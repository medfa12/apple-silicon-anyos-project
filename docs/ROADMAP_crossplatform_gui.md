...
> Read alongside `INSIGHTS_blackberry.md` and `COMPARISON_zhuowei.md`.

## 1. Executive Summary

The cross-platform TCG path (the Linux/Windows ARM target) can realistically reach a
**fast, interactive bash-on-serial kernel demo** — reproducing BlackBerry's 50s /
...
|---|---|---|
| Serial bash (keyboard works) | Yes | **Proven** (BlackBerry + our TCG baseline) |
...
- **Risks:** No IOMFB/GPU at T1, but boot may need to progress *past* PE console init — possibly
 ...
 possibly lazier console init) vs BlackBerry's 7195.
...
- **Steps:**
 ...
 WindowServer/Aqua. Enrich it (BlackBerry recipe): extract arm64e binaries from
...
backed by a fast interactive bash-on-serial kernel demo — exactly BlackBerry's achievement plus
